...
string answer(const string &line) {
  ...
  size_t pos = line.find("Simon says");
  if (pos == 0) {
    return line.substr(10); // 10 is the length of "Simon says"
  }
  return "";
}
...
void testAnswer() {
  ...
  assert(answer("Simon says hello") == " hello");
  assert(answer("Simon says goodbye") == " goodbye");
