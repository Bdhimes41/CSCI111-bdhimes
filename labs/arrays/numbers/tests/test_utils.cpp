...
TEST_CASE("Test Max & Min Functionality")
{
    ...
    SUBCASE("Sub Case 3: Single element")
    {
        int nums[] = {42};
        int max, min;
        find_max_min(nums, 1, max, min);
        CHECK(max == 42);
        CHECK(min == 42);
    }
    SUBCASE("Sub Case 4: Negative numbers")
    {
        int nums[] = {-3, -1, -4, -1, -5, -9, -2, -6, -5};
        int max, min;
        find_max_min(nums, 9, max, min);
...
TEST_CASE("Test Bubble Sort Functionality")
{
    ...
    SUBCASE("Sub Case 3: Reverse sorted array")
    {
        int nums[] = {5, 4, 3, 2, 1};
        bubble_sort(nums, 5);
        int expected[] = {1, 2, 3, 4, 5};
...
TEST_CASE("Find Sum Functionality")
{
    ...
    SUBCASE("Sub Case 3: All negative numbers")
    {
        int nums[] = {-1, -2, -3, -4, -5};
        long long sum = find_sum(nums, 5);
        CHECK(sum == -15);
    }
    SUBCASE("Sub Case 4: Large numbers")
    {
        int nums[] = {1000000000, 2000000000, 3000000000};
        long long sum = find_sum(nums, 3);
        CHECK(sum == 6000000000);
    }
}