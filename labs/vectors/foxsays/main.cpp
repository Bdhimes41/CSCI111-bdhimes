...
void eraseAnimalSound(vector<string> &recordings, string sound) {
  ...
  // FIXME3 - use iterator to erase all matching sound
    for (auto it = recordings.begin(); it != recordings.end(); ) {
        if (*it == sound) {
        it = recordings.erase(it); // erase returns the next element's iterator
        } else {
...
void test_splitString() {
  ...
  answer.clear();
  splitString(answer, "three word sentence");
  vector<string> expected2 = {"three", "word", "sentence"};
  assert(answer == expected2);
  answer.clear();
  splitString(answer, "four word sentence with more words");
  vector<string> expected3 = {"four", "word", "sentence", "with", "more", "words"};
...
void test_eraseAnimalSound() {
  ...
  // FIXME5: add 2 more test cases
    recordings = {"woof", "meow", "quack", "woof"};
    eraseAnimalSound(recordings, "woof");
    expected = {"meow", "quack"};
    assert(recordings == expected);
    recordings = {"tweet", "tweet", "chirp"};
    eraseAnimalSound(recordings, "tweet");
    expected = {"chirp"};
